...
y_np = np.cos(steps)  # Target data: cosine wave

# Plot the original data
plt.plot(steps, y_np, 'r-', label='Target (cos)')
plt.plot(steps, x_np, 'b-', label='Input (sin)')
plt.legend(loc='best')
plt.title('Sine Wave Input vs. Cosine Wave Target')
plt.show()
...
for step in range(Epoch):
    ...
    y_np = np.cos(steps)  # Target cosine wave
    ...
    # plt.text(steps[0], 1.1, f'Accuracy = {accuracy:.2f}', fontdict={'size': 12, 'color': 'green'})
